  ...
  >
    <div v-if="!windowData.isMaximized">
      ...
      <div
        ...
        @mousedown.left.stop="startResize($event, 'right')"
      ></div>
      <div
        class="resize-handle bottom"
        @mousedown.left.stop="startResize($event, 'bottom')"
      ></div>
      <div
        class="resize-handle left"
        @mousedown.left.stop="startResize($event, 'left')"
      ></div>
      <div
        class="resize-handle top-left"
        @mousedown.left.stop="startResize($event, 'top-left')"
      ></div>
      <div
        class="resize-handle top-right"
        @mousedown.left.stop="startResize($event, 'top-right')"
      ></div>
      <div
        class="resize-handle bottom-left"
        @mousedown.left.stop="startResize($event, 'bottom-left')"
      ></div>
      <div
        class="resize-handle bottom-right"
        @mousedown.left.stop="startResize($event, 'bottom-right')"
      ></div>
    </div>
    ...
    >
      ...
      <div class="window-controls">
        ...
        >
          <font-awesome-icon
            :icon="windowData.isMaximized ? 'window-restore' : 'window-maximize'"
          />
        </button>
  ...
  computed: {
    ...
    windowStyle() {
      ...
      const styles = {
        ...
        width:
          this.windowData.isMaximized && !this.isRestoring
            ? "100%"
  ...
  methods: {
      ...
      const onMouseMove = (moveEvent) => {
        ...
        if (
          ...
          if (this.windowData.isMaximized || this.windowData.previousPosition) {
            ...
            currentPosition = this.windowData.previousPosition ||
              prefs.pos || { x: 100, y: 100 };
        ...
        if (clientX < SNAP_ZONE_SIZE && clientY < TOP_SNAP_ZONE_Y) {
          ...
          clientX > innerWidth - SNAP_ZONE_SIZE &&
          clientY > BOTTOM_SNAP_ZONE_Y
        ) {
          newSnapTarget = {
            id: "br",
            x: "50%",
    ...
    startResize(event, direction) {
      ...
      const onMouseMove = (moveEvent) => {
        ...
        store.updateWindowSize(this.windowData.id, {
          width: newWidth,
          height: newHeight,
        });
...
.window-wrapper.is-restoring {
  transition: transform 0.2s cubic-bezier(0.4, 0, 0.2, 1),
    width 0.2s cubic-bezier(0.4, 0, 0.2, 1), height 0.2s cubic-bezier(0.4, 0, 0.2, 1);
}
