    ...
    <div class="account-list">
        ...
        <div v-for="account in accounts" :key="account.localId" class="account-item glass">
            ...
            <div class="account-actions">
                <button class="action-btn ui-hover" @click="toggleRevealAll(account.localId)"
                    :title="vault.revealedPasswords[account.localId] ? 'Ocultar Credenciais' : 'Revelar Credenciais'">
...
export default {
    ...
    methods: {
        ...
        returnAccountTypeImage(account) {
            ...
            return 'https://img.logo.dev/' + account.type.toLowerCase().split(' ')[0] + '.com?token=pk_PAz0kI4nRd6KqTrziA6zdw';
        },
        ...
        async copyUserAndHide(localId, text) {
            try {
                ...
                if (this.setResponse) {
                    this.setResponse("success", "Usuário copiado com sucesso! (Credenciais ocultadas)", false);
        ...
        async copyPassword(localId) {
            ...
            if (success && this.setResponse) {
                this.setResponse("success", "Senha copiada com sucesso! (Ocultada por segurança)", false);
                setTimeout(() => this.resetResponse(), 1500);
            }
        },
