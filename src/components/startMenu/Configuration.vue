<template>
    <div class="config-wrapper">
        <section class="config-section">
            <h3>Segurança</h3>
            <p>
                Para redefinir sua senha, enviaremos um link de recuperação para o seu
                e-mail.
            </p>
            <button class="btn btn-primary" :disabled="!isConnected || isLoading"
                :title="!isConnected ? 'Esta ação requer conexão com a internet.' : ''" @click="showResetModal = true">
        ...
        <section class="config-section">
            <h3>Conta</h3>
            <p>
                Esta é uma ação irreversível. Todos os seus dados, projetos e
                informações do cofre serão permanentemente excluídos do Kadem.
            </p>
            <button class="btn btn-red" :disabled="!isConnected"
                :title="!isConnected ? 'Esta ação requer conexão com a internet.' : ''" @click="showDeleteModal = true">
                Excluir conta
            </button>
        </section>

        <ConfirmationModal v-model="showResetModal"
            message="Você confirma que deseja solicitar a redefinição da sua senha? "
            description="Um e-mail será enviado para sua conta." confirm-text="Confirmar"
            @confirmed="handleRequestPasswordReset" @cancelled="showResetModal = false" />
